...
const {
  getParams,
} = require('../../middleware/user/active/getdetails.middleware');
const { register } = require('../../middleware/user/active/singUp.middleware');
// -- sign in account
const {
  checkEmail,
} = require('../../middleware/user/sign_in/checkEmail.middleware');
const {
  populateUser,
...
const {
  createInfo,
} = require('../../middleware/user/sign_up/CreateInfo.middleware');
const {
  createContact,
} = require('../../middleware/user/sign_up/CreateContacts.middleware');
const {
  createPassword,
} = require('../../middleware/user/sign_up/CreatePassword.middleware');
const {
  createRole,
} = require('../../middleware/user/sign_up/CreateRole.middleware');
const {
  createToken,
} = require('../../middleware/user/sign_up/CreateToken.middleware');
const {
  existUser,
} = require('../../middleware/user/sign_up/existUser.middleware');
// -- reset password account
const {
  checkUserExist,
} = require('../../middleware/user/reset_password/CheckUserExist.middleware');
...
const {
  signInInputs,
} = require('../../validation/validator/auth-user/sign-InInputs');
// --- validation sign up inputs
const {
  signUpInputs,
} = require('../../validation/validator/auth-user/sign-upInputs');

// ----- swagger auth descriptions
const {
  tagNameUserAuth,
} = require('../../utility/node-mailer/EmailsTemplates/Auth_user.swagger.tag');

const {
  signupSwagger,
  signingSwagger,
  activateAccountSwagger,
  authTestSwagger,
  requestResetPasswordSwagger,
  resetUserPasswordSwagger,
  fetchGoogleUserInfoSwagger,
  accessGoogleAuthSwagger,
  resetPasswordPageSwagger,
} = require('../../swagger/middleware/user/auth/auth_user_description.swagger');
const {
  resetInputs,
  resetPasswordInputs,
} = require('../../validation/validator/auth-user/resetPasswordInputs');
const {
  CheckEmailSent,
} = require('../../middleware/user/reset_password/EmailsSents.middleware');
const {
  resetPasswordFile,
} = require('../../controller/user/auth/resetPasswordFile.controller');
const {
  finderByEmail,
} = require('../../middleware/user/reset_password/FindUserByEmail.middleware');
const resetPasswordValidation = require('../../controller/user/auth/resetPasswordValidation.controller');

const {
  handleAuthorizationCode,
} = require('../../middleware/provider/google/authCode.middleware');
const {
  handleGoogleSignIn,
} = require('../../middleware/provider/google/googleSignIn.middleware');
const {
  fetchGoogleUserInfo,
} = require('../../middleware/provider/google/googleUserInfo.middleware');
const handleGoogleAuth = require('../../controller/user/provider/accessGoogle.controller');
const {
  generateGoogleAuthUrl,
} = require('../../middleware/provider/google/googleAccess.middleware');
