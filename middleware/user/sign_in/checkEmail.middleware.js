...
exports.checkEmail = async (req, res, next) => {
  ...
  try {
    ...
    if (!contact) {
      return res
        .status(400)
        .json({ message: 'your account is not exist please try to sign up' });
    }
    // ----- find user exist
    const user = await User.findOne({ contact: contact.id }).populate([
      'password',
      'contact',
      'role',
      'info',
    ...
    if (!user) {
      return res.status(400).json({
        message: 'your account is not activate please check your email',
      });
    }
