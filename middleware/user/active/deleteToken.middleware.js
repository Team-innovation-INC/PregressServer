  ...
  try {
    ...
    if (result) {
      return res.status(200).send({
        message:
          'your account is activate with success you can now redirected to our site and sign in ',
      });
    }
    // ----- case a problem message for a problem
    return res.status(403).send({
      message:
        'oops we activate your account but if you get at the future some problem please contact us',
    });
  } catch (error) {
