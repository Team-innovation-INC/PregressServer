...
const resetPassword = async (req, res) => {
  ...
  const activeLink = `${basedUrl}/api/auth/reset-password?token=${token}`;
  // ----- create html file for reset password account
  const html = getHtmlFile(userEmail, activeLink, 'resetPassword.hbs');
  // ----- create information related to user email
  const subject = 'reset password account';
  const text = `please click to this link to reset your password account${activeLink}`;
  ...
  try {
    ...
    return res
      .status(200)
      .send({ message: 'please check your email please and thanks' });
  } catch (error) {
