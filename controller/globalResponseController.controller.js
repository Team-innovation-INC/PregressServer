 ...
 * @param {import('express').Request & { data?: any, status: number, errorMessage?: string }} req - The Express request object.
...
const globalResponseController = (req, res) => {
  try {
     ...
     */
    const { data, status, errorMessage } = req;
    console.log(data, status, errorMessage,"data")
    ...
    const message = errorMessage ?? `${routeName}.${success ? 'success' : 'failed'}`;
